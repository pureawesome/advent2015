...
class Six
  ...
  def reset(int)
    @lights = Array.new(1_000_000, int)
  end

  def change(str)
    arr = str.scan(/\d+/).map(&:to_i)
    x_array = (arr[0]..arr[2]).to_a
    y_array = (arr[1]..arr[3]).to_a
    coords = x_array.product(y_array)
    action = str.start_with?('turn on') ? 'on' : str.start_with?('turn off') ? 'off' : 'toggle'
...
class TestSix < Minitest::Test
  ...
  def test_one
    skip
    @six.reset(0)
    assert_equal 1_000_000, @six.all_change('turn on 0,0 through 999,999')
  end

  def test_two
    skip
    @six.reset(0)
    assert_equal 1000, @six.all_change('toggle 0,0 through 999,0')
  end

  def test_three
    skip
    @six.reset(1)
    assert_equal 999_996, @six.all_change('turn off 499,499 through 500,500')
  end

  def test_four
    # skip
    @six.reset(0)
    assert_equal 1, @six.all_change('turn on 0,0 through 0,0')
  end

  def test_four
    # skip
    @six.reset(0)
    assert_equal 2_000_000, @six.all_change('toggle 0,0 through 999,999')
  end
end

@run = Six.new
@instr = 'turn off 660,55 through 986,197
